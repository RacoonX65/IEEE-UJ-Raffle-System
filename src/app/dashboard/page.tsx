...
export default function EnhancedDashboard() {
  ...
  const [showTicketSalesForm, setShowTicketSalesForm] = useState(false)
  ...
  const [filteredTickets, setFilteredTickets] = useState<TicketEntry[]>([])
  ...
  const generateQRCode = async (ticket: TicketEntry) => {
    try {
      ...
      if (response.ok) {
        ...
        alert(`Failed to generate QR code: ${errorData.details || errorData.error || 'Unknown error'}`)
      }
    } catch (error) {
      console.error('Error generating QR code:', error)
      alert(`Error generating QR code: ${error instanceof Error ? error.message : 'Network error'}`)
    }
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-900">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-8">
          <button
            onClick={() => setShowTicketSalesForm(true)}
            className="flex items-center justify-center px-6 py-3 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 rounded-2xl text-white font-medium transition-all duration-200 shadow-lg hover:shadow-xl transform hover:scale-105"
          >
          ...
          <button
            ...
            className="flex items-center justify-center px-6 py-3 bg-gradient-to-r from-orange-600 to-red-600 hover:from-orange-700 hover:to-red-700 rounded-2xl text-white font-medium transition-all duration-200 shadow-lg hover:shadow-xl transform hover:scale-105"
          >
          ...
          <button
            ...
            className="flex items-center justify-center px-6 py-3 bg-gradient-to-r from-pink-600 to-purple-600 hover:from-pink-700 hover:to-purple-700 rounded-2xl text-white font-medium transition-all duration-200 shadow-lg hover:shadow-xl transform hover:scale-105"
          >
        ...
        {data && data.recentEntries && data.recentEntries.length > 0 && (
          <TicketSearchFilter
            tickets={data.recentEntries}
            onFilteredTickets={setFilteredTickets}
          />
        )}
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody className="divide-y divide-white/10">
                  {(filteredTickets.length > 0 ? filteredTickets : data.recentEntries)?.slice(0, 20).map((entry: TicketEntry, index: number) => (
                    <tr key={index} className="hover:bg-white/5 transition-colors duration-200">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          {entry.paymentMethod === 'Cash' || entry.paymentStatus === 'VERIFIED' ? (
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center space-x-2">
                          ...
                          {/* Payment Verification Button - only show for non-cash payments */}
                          {entry.paymentMethod !== 'Cash' && entry.paymentStatus !== 'VERIFIED' && (
                            <button
                              onClick={() => {
                              ...
                              className="p-2 bg-orange-600 hover:bg-orange-700 rounded-lg text-white transition-colors duration-200"
      ...
      <TicketSalesForm
        isOpen={showTicketSalesForm}
        onClose={() => setShowTicketSalesForm(false)}
        onTicketSold={() => {
          fetchData() // Refresh dashboard data after ticket is sold
        }}
      />
      ...
      {selectedTicket && (
        <PaymentVerification
          ...
          ticket={{
            ...
            buyerName: selectedTicket.name,
            paymentMethod: selectedTicket.paymentMethod,
            amount: 50 // Default ticket price
          }}
        />
      )}
